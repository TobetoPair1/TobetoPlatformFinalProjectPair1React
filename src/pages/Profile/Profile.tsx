...
const Profile = (props: Props) => {
  ...
  const [foreignLanguages, setForeignLanguages] = useState<Paginate<ForeignLanguageGetListResponseModel>>()
  const [certificates, setCertificates] = useState<Paginate<CertificateGetListResponseModel>>()
  const [socialMedias, setSocialMedias] = useState<Paginate<SocialMediaGetListResponseModel>>()


  const [personalInfo, setPersonalInfo] = useState<PersonalInfoGetResponseModel>()
  const fetchUserData = async () => {

    const skillResponse = await SkillService.GetListByUserId({ PageIndex: 0, PageSize: 5 }, platform.user.id)
    const personalInfoResponse = await PersonalInfoService.GetByUserId(platform.user.id)
    const foreignLanguageResponse = await ForeignLanguageService.GetListByUserId({ PageIndex: 0, PageSize: 5 }, platform.user.id)
    const certificateResponse = await CertificateService.getListByUserId({ PageIndex: 0, PageSize: 5 }, platform.user.id)
    const socialMediaResponse = await SocialMediaService.getListByUserId({ PageIndex: 0, PageSize: 5 }, platform.user.id)
            ...
            <div className="row">
              <div className="col-12">
                <div className="cv-box">
                  <div className="cv-pp ">
                    ...
                    </div><span style={{ boxSizing: 'border-box', display: 'inline-block', overflow: 'hidden', width: 'initial', height: 'initial', background: 'none', opacity: 1, border: '0px', margin: '0px', padding: '0px', position: 'relative', maxWidth: '100%' }}><span style={{ boxSizing: 'border-box', display: 'block', width: 'initial', height: 'initial', background: 'none', opacity: 1, border: '0px', margin: '0px', padding: '0px', maxWidth: '100%' }}><img alt="" aria-hidden="true" src="data:image/svg+xml,%3csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20version=%271.1%27%20width=%27128%27%20height=%27128%27/%3e" style={{ display: 'block', maxWidth: '100%', width: 'initial', height: 'initial', background: 'none', opacity: 1, border: '0px', margin: '0px', padding: '0px' }} /></span><img alt="" src="https://tobeto.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fimages.19a45d39.png&w=256&q=75" decoding="async" data-nimg="intrinsic" className="cv-pp-img rounded-circle " srcSet={personalInfo?.profileImageUrl? personalInfo?.profileImageUrl :"https://tobeto.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fimages.19a45d39.png&w=128&q=75  1x,https://tobeto.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fimages.19a45d39.png&w=256&q=75  2x"} style={{ position: 'absolute', inset: '0px', boxSizing: 'border-box', padding: '0px', border: 'none', margin: 'auto', display: 'block', width: '0px', height: '0px', minWidth: '100%', maxWidth: '100%', minHeight: '100%', maxHeight: '100%' }} /></span>
                  </div>
                  <div className="cv-info cv-padding">
                    <div className="info-box">
                      <div className="info-icon name" />
                      <div className="info-text"><span className="header">Ad Soyad: </span><span className="text">{platform?.user.firstName + " " + platform?.user.lastName}</span></div>
                    </div>
                    <div className="info-box">
                      <div className="info-icon date" />
                      <div className="info-text"><span className="header">Doğum Tarihi: </span><span className="text">{new Date(personalInfo?.birthDate ? personalInfo.birthDate : " ").toLocaleDateString()}</span></div>
                    </div>
                    <div className="info-box">
                      <div className="info-icon mail" />
                      <div className="info-text"><span className="header">E-Posta: </span><span className="text">{platform?.user?.email}</span></div>
                    </div>
                    <div className="info-box">
                      <div className="info-icon phone" />
                      <div className="info-text"><span className="header">Telefon: </span><span className="text">{personalInfo?.phoneNumber}</span></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="col-12">
                <div className="cv-box cv-padding">
                  ...
                  <div className='skills'>
                    {
                      skills?.count ? skills?.count > 0 ? skills?.items?.map((item) => <div key={item.id} className='skill'>{item.name}</div>)
                        : <div>Henüz bir yetkinlik eklemedin.</div> : <div>Henüz bir yetkinlik eklemedin.</div>
                    }
              ...
              <div className="col-12">
                <div className="cv-box cv-padding">
                  ...
                  <div className="my-langs">
                    {foreignLanguages?.count ? foreignLanguages?.count > 0 ? foreignLanguages?.items.map((item) => <div className="lang w-100" key={item.id}>
                      <div className="lang-info">
                        <div className="lang-title ">
                          <div className="d-flex flex-column"><span className="lang-name">{item.name}</span><span className="lang-degree">{item.level}</span></div>
                        </div>
                      </div><span className="lang-degree-symbol main-lang" />
                    </div>) : <div>Henüz yabancı dil eklemedin </div> : <div>Henüz yabancı dil eklemedin </div>
                    }
                  </div>
                </div>
              </div>
              <div className="col-12">
                <div className="cv-box cv-padding">
                  ...
                  <div className="row">
                    {
                      certificates?.count ? certificates?.count > 0 ? certificates?.items.map((item) =>
                        <div className="skills"><span id="certificate_name" className="skill d-flex justify-content-between"><span className="me-2 text-truncate ">{item.name}</span><span className="me-0 png_icon text - center" /></span></div>) : <div>Henüz sertifika eklemedin</div> : <div>Henüz sertifika eklemedin</div>
                    }
                  </div>
                </div>
              </div>
              <div className="col-12">
                <div className="cv-box cv-padding">
                  ...
                  <div className="cv-social-media">
                    {
                      socialMedias?.count ? socialMedias?.count > 0 ? socialMedias?.items.map((item) =>
                        <a key={item.id} className={"cv-" + (item.name)} target="_blank" href={item.url} />
                      ) : <div> Henüz sosyal medya eklemedin </div> : <div> Henüz sosyal medya eklemedin </div>
                    }
                  </div>
                </div>
              </div>
