...
export const getApplications = createAsyncThunk(
    ...
    async function GetApplications(data:{id:string,pageRequest:PageRequestModel}) {
        const applications = (await ApplicationService.GetListByUserId(data.pageRequest, data.id)).data;
        return applications;
    }
...
export const getExams = createAsyncThunk(
    "platform/getExams",
    async function GetApplications(data:{id:string,pageRequest:PageRequestModel}) {
        const exams = (await ExamService.GetListByUserId(data.pageRequest, data.id)).data;
        return exams;
    }
...
export const getCourses = createAsyncThunk(
    "platform/getCourses",
    async function GetApplications(data:{id:string,pageRequest:PageRequestModel}) {
        const courses = (await CourseService.GetListByUserId(data.pageRequest, data.id)).data;
        return courses;
    }
...
export const getAnnouncements = createAsyncThunk(
    ...
    async function GetApplications(pageRequest:PageRequestModel) {
...
const platformSlice = createSlice({
    ...
    reducers: {
        ...
        getUser: (state) => {  
            const fullName = state.token["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"].split(' ');
            const id = state.token["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier"];
            state.user = { id: id, email: state.token.email, firstName: fullName[0], lastName: fullName[1], isInstructor: false }
        }
    },
